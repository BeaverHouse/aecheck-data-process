...
| alter_character    | varchar(20)  | NULL     | -                 | 대응하는 이시층 캐릭터 코드             |
| seesaa_url         | varchar(500) | NULL     | -                 | Seesaa Wiki URL                         |
| aewiki_url         | varchar(500) | NULL     | -                 | AE Wiki URL                             |
| update_date        | date         | NULL     | -                 | 업데이트 날짜                           |
| custom_manifest    | bool         | NULL     | false             | 결실 현현 여부                          |
| personalities_data | jsonb        | NULL     | '[]'::jsonb       | 퍼스널리티 데이터 (JSON 배열)           |
| dungeons_data      | jsonb        | NULL     | '[]'::jsonb       | 던전 데이터 (JSON 배열)                 |
| buddy_data         | jsonb        | NULL     | -                 | 버디 데이터 (JSON 객체)                 |
| tier               | varchar(10)  | NULL     | -                 | 티어 등급 (Super OP/OP, null이면 일반)  |
...
- idx_characters_character_code ON (character_code)
- idx_characters_alter_character ON (alter_character) WHERE alter_character IS NOT NULL

### buddies

버디(Sidekick/バディ) 정보를 저장합니다.

| 컬럼명       | 타입         | NULL     | 기본값            | 설명                                                 |
| ------------ | ------------ | -------- | ----------------- | ---------------------------------------------------- |
| buddy_id     | varchar(10)  | NOT NULL | -                 | 버디 ID                                              |
| character_id | varchar(10)  | NULL     | -                 | 파트너 캐릭터 ID                                     |
| get_path     | varchar(20)  | NULL     | -                 | 획득 경로 (파트너 캐릭터가 있다면 NULL, 아니면 키값) |
| seesaa_url   | varchar(500) | NULL     | -                 | Seesaa Wiki URL                                      |
| aewiki_url   | varchar(500) | NULL     | -                 | AE Wiki URL                                          |
| created_at   | timestamp    | NOT NULL | CURRENT_TIMESTAMP | 생성일                                               |
| updated_at   | timestamp    | NULL     | -                 | 수정일                                               |
...
- idx_buddies_character_id ON (character_id)
